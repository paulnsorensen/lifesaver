...
describe Lifesaver::Notification::IndexingGraph do
  ...
  describe '#add_unvisited_associations' do
    before do
      models = [Model.new(1), Model.new(2)]
      indexing_graph.stub(:load_associations_for_model).and_return(models)
    end

    context 'when model has more models to notify' do
      it 'adds models to loader' do
        indexing_graph.stub(:model_needs_to_notify?).and_return(true)
        indexing_graph.stub(:add_model_to_loader)
        expect(indexing_graph).to receive(:add_model_to_loader).twice
        indexing_graph.send(:add_unvisited_associations, Model.new(4))
      end
    end

    context 'when model has no models to notify' do
      it 'adds models to queue' do
        indexing_graph.stub(:model_needs_to_notify?).and_return(false)
        indexing_graph.stub(:push)
        expect(indexing_graph).to receive(:push_model).twice
        indexing_graph.send(:add_unvisited_associations, Model.new(4))
      end
    end
  end
